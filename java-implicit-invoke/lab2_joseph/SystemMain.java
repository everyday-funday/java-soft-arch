...
class SystemMain {
	...
	public static void main(String args[]) {
		...
		try {
			...
			// Shared Database (Database.java) from TXT files
			db = new DataBase(studentFileName, courseFileName);

			// Initialize the handlers to specific events from the bus.
			ListAllStudentsHandler objCommandEventHandler1 =
				new ListAllStudentsHandler(
					db,
					EventBus.EV_LIST_ALL_STUDENTS,		//Subscribing to EV_LIST_ALLSTUDENTS
					EventBus.EV_SHOW);					// Ouput EVSHOW
			 ...
			 *  Modification 3 -
			 *
			 *  Chaining CheckConflictHandler and RegisterStudentHandler in series
			 *  EV_CHECK_COURSE_CONFLICT event triggers
			 *  EV_REGISTER_STUDENT which triggers
			 *  EV_SHOW
			 *
			 */
			CheckConflictHandler objCommandEventHandler6 =
				new CheckConflictHandler(
					db,
			...
			ClientLogOutput objLogOutput = new ClientLogOutput();
